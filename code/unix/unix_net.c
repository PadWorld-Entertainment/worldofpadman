...
#include <sys/uio.h>
#include <errno.h>
#include <fcntl.h> // for nonblocking tcp
...
static int				WMsocket[MAX_WOPMASTER_SERVERS];
	...
	case HS_SOCKETCLOSED:
		...
		val=fcntl(WMsocket[master], F_GETFL, 0);
		val |= O_NONBLOCK|O_NDELAY;
		if(0 != fcntl(WMsocket[master],F_SETFL,&val))
		{
			Com_Printf("[WoPMaster] setting nonblocking failed\n");
			close(WMsocket[master]);
			WMsocket[master] = -1;
			return -1;
		}
