...
static int Export_BotLibSetup(void) {
	...
	botlibglobals.maxclients = LibVarInteger("maxclients", "128", 0, MAX_CLIENTS);
...
static int BotExportTest(int parm0, char *parm1, vec3_t parm2, vec3_t parm3) {
	...
	if (parm0 & BUTTON_USE) {
		...
		if (botlibglobals.runai) {
			botimport.Print(PRT_MESSAGE, "started AI\n");
		} else {
			botimport.Print(PRT_MESSAGE, "stopped AI\n");
		}
	...
	if (bsptrace.fraction < 1.0) {
		AAS_DrawPlaneCross(bsptrace.endpos, bsptrace.plane.normal,
