...
static void AAS_CalculateAreaTravelTimes(void) {
	...
	for (i = 0; i < aasworld.numareas; i++) {
		...
		size += settings->numreachableareas * PAD(revreach->numlinks * sizeof(unsigned short), sizeof(uintptr_t));
	}
...
static void AAS_InitRoutingUpdate(void) {
	...
	aasworld.clusterCacheHeap =
		(aas_routingupdate_t **)GetClearedMemory(maxreachabilityareas * sizeof(aas_routingupdate_t *));
...
void AAS_InitRouting(void) {
	...
	routingcachesize = 0;
	max_routingcachesize = 1024 * LibVarInteger("max_routingcache", "4096", 0, 65536);
...
void AAS_FreeRoutingCaches(void) {
	...
	if (aasworld.clusterCacheHeap)
		FreeMemory(aasworld.clusterCacheHeap);
...
static void ClusterCacheHeapPrint(void) {
	...
	for (i = 0; i < aasworld.clusterCacheHeapSize; i++)
		Log_Write("%ld->%d ", aasworld.clusterCacheHeap[i] - aasworld.areaupdate,
...
static int ClusterCacheHeapRightChild(int nodeID) {
	return 2 * nodeID + 2;
}

static int ClusterCacheHeapParent(int nodeID) {
	return ((nodeID - 1) / 2);
}

static qboolean ClusterCacheHeapEmpty(void) {
	return (aasworld.clusterCacheHeapSize <= 0);
}

static void ClusterCacheHeapUpdatePos(int nodeID) {
	aasworld.clusterCacheHeap[nodeID]->heapPos = nodeID;
}

static void ClusterCacheHeapSiftDown(int nodeID) {
	...
	if (aasworld.clusterCacheHeap[nodeID]->tmptraveltime > aasworld.clusterCacheHeap[minChildID]->tmptraveltime) {
...
static aas_routingupdate_t *ClusterCacheHeapExtractMin(void) {
...
static qboolean ClusterCacheHeapSiftUp(int nodeID) {
	int parentID;
	aas_routingupdate_t *tmp;

	if (nodeID != 0) {
		parentID = ClusterCacheHeapParent(nodeID);
		if (aasworld.clusterCacheHeap[parentID]->tmptraveltime > aasworld.clusterCacheHeap[nodeID]->tmptraveltime) {
			// swap
...
static void ClusterCacheHeapInsert(aas_routingupdate_t *item) {
...
static void ClusterCacheHeapDecreaseKey(aas_routingupdate_t *item) {
	...
	while (!ClusterCacheHeapEmpty()) {
		...
		for (i = 0, revlink = revreach->first; revlink; revlink = revlink->next, i++) {
			...
			if (aasworld.areaupdate[clusterareanum].tmptraveltime == TT_INF) {
				...
				aasworld.areaupdate[clusterareanum].areatraveltimes =
					aasworld
