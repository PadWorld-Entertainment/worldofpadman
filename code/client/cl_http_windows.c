...
qboolean CL_HTTP_Init(void) {
	...
	const char *windowsVersion = GetVersionEx(&osvi) ? va("Windows %lu.%lu (build %lu)", osvi.dwMajorVersion,
...
void CL_HTTP_BeginDownload(const char *remoteURL) {
	...
	hUrl = InternetOpenUrlA(hInternet, remoteURL, va("Referer: ioQ3://%s\r\n", NET_AdrToString(clc.serverAddress)), -1,
							INTERNET_FLAG_HYPERLINK | INTERNET_FLAG_NO_CACHE_WRITE | INTERNET_FLAG_NO_COOKIES |
								INTERNET_FLAG_NO_UI | INTERNET_FLAG_RESYNCHRONIZE | INTERNET_FLAG_RELOAD,
							0);
