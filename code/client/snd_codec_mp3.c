...
// Q3 MP3 codec
snd_codec_t mp3_codec = {".mp3", S_MP3_CodecLoad, S_MP3_CodecOpenStream, S_MP3_CodecReadStream, S_MP3_CodecCloseStream,
						 NULL};
	...
	while (1) {
		...
		while (1) {
			...
			if (!stream->info.samples) {
				...
				if (stream->info.rate != madheader.samplerate || stream->info.channels != MAD_NCHANNELS(&madheader))
					return -1;
			}
...
int S_MP3_PCMCopy(byte *buf, struct mad_pcm *pcm, int bufofs, int sampleofs, int bytecount, int *wrotecnt) {
	int written = 0;
	signed int sample;
...
int S_MP3_Decode(snd_stream_t *stream) {
	...
	if (stream->info.rate != pcm->samplerate || stream->info.channels != pcm->channels) {
		return -1;
	}
	...
	samplecount = S_MP3_PCMCopy(mp3info->dest, pcm, mp3info->destlen, 0, needcount, &wrote);
	mp3info->destlen += wrote;

	if (samplecount < pcm->length) {
		// Not all samples got copied. Copy the rest into the pcm buffer.
		samplecount = S_MP3_PCMCopy(mp3info->pcmbuf, pcm, mp3info->buflen, samplecount,
									mp3info->pcmbufsize - mp3info->buflen, &wrote);
