...
*/
void Frame_yuv_to_rgb24(const unsigned char *y, const unsigned char *u, const unsigned char *v, int width, int height,
						int y_stride, int uv_stride, int yWShift, int uvWShift, int yHShift, int uvHShift,
						unsigned int *output) {
	int i, j, uvI;
	long r, g, b, YY;
...
e_status CIN_RunCinematic(int handle) {
	...
	if (cinTable[currentHandle].fileType == FT_OGM) {
		...
		if (Cin_OGM_Run(&cin, cinTable[currentHandle].startTime == 0
								  ? 0
	...
	// FIXME? CL_ScaledMilliseconds already uses com_timescale (so I can't see that the com_timescale in here makes any
	// sense at all O_o)
...
char *
S_FileExtension(const char *fni); // from snd_codec.c (just a nice implementation of getting a point to the extention)
