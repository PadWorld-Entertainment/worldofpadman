...
typedef struct {
	...
	qboolean videoStreamIsXvid; // FIXME: atm there isn't realy a check for this (all "video" streams are handelt as
								// xvid, because xvid support more than one "subtype")
#ifdef USE_CIN_XVID
...
static qboolean loadAudio(cinematics_t *cin) {
	...
	while (anyDataTransferred &&
		   g_ogm.currentTime + MAX_AUDIO_PRELOAD > (int)(g_ogm.vd.granulepos * 1000 / g_ogm.vi.rate)) {
		...
		if ((samples = vorbis_synthesis_pcmout(&g_ogm.vd, &pcm)) > 0) {
			...
			for (i = 0; i < samplesNeeded; ++i) {
				ptr[0] = (left[i] >= -1.0f && left[i] <= 1.0f) ? left[i] * 32767.f
															   : 32767 * ((left[i] > 0.0f) - (left[i] < 0.0f));
				ptr[1] = (right[i] >= -1.0f && right[i] <= 1.0f) ? right[i] * 32767.f
	...
	while (!r && (ogg_stream_packetout(&g_ogm.os_video, &op))) {
		...
		if (g_ogm.xvid_dec_stats.type == XVID_TYPE_VOL) {
			...
			if (g_ogm.outputBufferSize < g_ogm.xvid_dec_stats.data.vol.width * g_ogm.xvid_dec_stats.data.vol.height) {
				...
				g_ogm.outputBuffer =
					(unsigned char *)malloc(g_ogm.outputBufferSize * 4); // FIXME? should the 4 stay for BPP?
				if (g_ogm.outputBuffer == NULL) {
		...
		if (g_ogm.xvid_dec_stats.type > 0) {
			...
			++g_ogm.VFrameCount;
			//			Com_Printf("frame infos: %d %d %d\n", xvid_dec_stats.data.vop.general,
			//xvid_dec_stats.data.vop.time_base, xvid_dec_stats.data.vop.time_increment); 			Com_Printf("frame info time:
			//%d (Frame# %d, %d)\n", xvid_dec_stats.data.vop.time_base, VFrameCount,
			//(int)(VFrameCount*Vtime_unit/10000000));
		}
