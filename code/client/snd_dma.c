...
=================
*/
static void S_SpatializeOrigin(vec3_t origin, int master_vol, int *left_vol, int *right_vol) {
	vec_t dot;
	vec_t dist;
...
============
*/
static void S_Base_RawSamples(int stream, int samples, int rate, int width, int channels, const byte *data, float volume,
					   int entityNum) {
	int i;
	int src, dst;
	float scale;
...
static void S_OpenBackgroundStream(const char *filename) {
	...
	if (s_backgroundStream->info.channels != 2 || s_backgroundStream->info.rate != 44100) {
		Com_Printf(S_COLOR_YELLOW "WARNING: music file %s is not 44kHz stereo\n", filename);
	}
}
