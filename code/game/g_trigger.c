...
void SP_trigger_hurt(gentity_t *self) {
	...
	self->noise_index = G_SoundIndex("sound/world/triggers/hurt");
...
static void trigger_exit_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	if (other && other->client && !(other->r.svFlags & SVF_BOT)) {
		...
		level.intermissionQueued =
			level.time - INTERMISSION_DELAY_TIME; // NOTE: on trigger we want to exit faster (on limit-hit there is a
												  // delay) ... and the LogExit wont work if it is called on every touch
												  // (overwrites because it always resets level.intermissionQueued)
	}
}
