...
int httpConnectTimeoutSecond = 5;
int httpTimeoutSecond = 5;

void HTTP_SetTimeouts(int connectTimeoutS, int readWriteTimeoutS) {
	httpConnectTimeoutSecond = connectTimeoutS;
	httpTimeoutSecond = readWriteTimeoutS;
}
...
#ifndef USE_HTTP
inline int HTTP_Init(void) {
	return 0;
}

inline void HTTP_Close(void) {
}

inline int HTTP_Execute(const char *mode, const char *url, const char *headers, const char *body,
						void (*writeCallback)(unsigned char *, int)) {
	return 0;
}
#endif /* USE_HTTP */
