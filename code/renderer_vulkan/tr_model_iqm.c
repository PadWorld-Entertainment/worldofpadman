	...
	for (i = 0; i < header->num_vertexarrays; i++, vertexarray++) {
		...
		case IQM_BLENDINDEXES:
			if (blendIndexesType == IQM_INT) {
				...
				memcpy(iqmData->blendIndexes, (byte *)header + vertexarray->offset, n * sizeof(byte));
			}
			break;
		case IQM_BLENDWEIGHTS:
			if (blendWeightsType == IQM_FLOAT) {
				memcpy(iqmData->blendWeights.f, (byte *)header + vertexarray->offset, n * sizeof(float));
			} else {
				memcpy(iqmData->blendWeights.b, (byte *)header + vertexarray->offset, n * sizeof(byte));
			}
			break;
		case IQM_COLOR:
			memcpy(iqmData->colors, (byte *)header + vertexarray->offset, n * sizeof(byte));
			break;
		}
	}
...
void RB_IQMSurfaceAnim(surfaceType_t *surface) {
	...
	for (i = 0; i < surf->num_vertexes; i++, outXYZ++, outNormal++, outTexCoord++, outColor++) {
		...
		(*outXYZ)[0] = vtxMat[0] * data->positions[3 * vtx + 0] + vtxMat[1] * data->positions[3 * vtx + 1] +
					   vtxMat[2] * data->positions[3 * vtx + 2] + vtxMat[3];
		(*outXYZ)[1] = vtxMat[4] * data->positions[3 * vtx + 0] + vtxMat[5] * data->positions[3 * vtx + 1] +
					   vtxMat[6] * data->positions[3 * vtx + 2] + vtxMat[7];
		(*outXYZ)[2] = vtxMat[8] * data->positions[3 * vtx + 0] + vtxMat[9] * data->positions[3 * vtx + 1] +
					   vtxMat[10] * data->positions[3 * vtx + 2] + vtxMat[11];
		(*outXYZ)[3] = 1.0f;

		(*outNormal)[0] = nrmMat[0] * data->normals[3 * vtx + 0] + nrmMat[1] * data->normals[3 * vtx + 1] +
						  nrmMat[2] * data->normals[3 * vtx + 2];
		(*outNormal)[1] = nrmMat[3] * data->normals[3 * vtx + 0] + nrmMat[4] * data->normals[3 * vtx + 1] +
						  nrmMat[5] * data->normals[3 * vtx + 2];
		(*outNormal)[2] = nrmMat[6] * data->normals[3 * vtx + 0] + nrmMat[7] * data->normals[3 * vtx + 1] +
