...
static void R_LoadLightmaps(lump_t *l) {
	...
	for (i = 0; i < tr.numLightmaps; i++) {
		...
		tr.lightmaps[i] =
			R_CreateImage(va("*lightmap%d", i), image, LIGHTMAP_SIZE, LIGHTMAP_SIZE, qfalse, qfalse, GL_CLAMP);
	}
...
static void ParseTriSurf(dsurface_t *ds, drawVert_t *verts, msurface_t *surf, int *indexes) {
	...
	tri = (srfTriangles_t *)ri.Hunk_Alloc(
		sizeof(*tri) + numVerts * sizeof(tri->verts[0]) + numIndexes * sizeof(tri->indexes[0]), h_low);
			...
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->width - 1; l++) {
					...
					if (m)
						...
						row = 0;
					grid2 = R_GridInsertColumn(grid2, l + 1, row, grid1->verts[k + 1 + offset1].xyz,
			...
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->height - 1; l++) {
					...
					if (m)
						...
						column = 0;
					grid2 = R_GridInsertRow(grid2, l + 1, column, grid1->verts[k + 1 + offset1].xyz,
											grid1->widthLodError[k + 1]);
					grid2->lodStitched = qfalse;
					s_worldData.surfaces[grid2num].data = (surfaceType_t *)(void *)grid2;
					return qtrue;
			...
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->width - 1; l++) {
					...
					if (m)
						...
						row = 0;
					grid2 = R_GridInsertColumn(grid2, l + 1, row, grid1->verts[grid1->width * (k + 1) + offset1].xyz,
			...
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->height - 1; l++) {
					...
					if (m)
						...
						column = 0;
					grid2 = R_GridInsertRow(grid2, l + 1, column, grid1->verts[grid1->width * (k + 1) + offset1].xyz,
											grid1->heightLodError[k + 1]);
					grid2->lodStitched = qfalse;
					s_worldData.surfaces[grid2num].data = (surfaceType_t *)(void *)grid2;
					return qtrue;
