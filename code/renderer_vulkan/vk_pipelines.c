...
enum Vk_Shadow_Phase { SHADOWS_RENDERING_DISABLED, SHADOWS_RENDERING_EDGES, SHADOWS_RENDERING_FULLSCREEN_QUAD };
...
static void vk_create_pipeline(const struct Vk_Pipeline_Def *def, VkPipeline *pPipeLine) {
	...
	shaderStages[1].pSpecializationInfo = (def->state_bits & GLS_ATEST_BITS) ? &specialization_info : NULL;
	...
	input_assembly_state.topology =
		def->line_primitives ? VK_PRIMITIVE_TOPOLOGY_LINE_LIST : VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
	...
	rasterization_state.polygonMode =
		(def->state_bits & GLS_POLYMODE_LINE) ? VK_POLYGON_MODE_LINE : VK_POLYGON_MODE_FILL;
	...
	depth_stencil_state.depthCompareOp =
		(def->state_bits & GLS_DEPTHFUNC_EQUAL) ? VK_COMPARE_OP_EQUAL : VK_COMPARE_OP_LESS_OR_EQUAL;
	...
	if (def->shadow_phase == SHADOWS_RENDERING_EDGES) {
		...
		depth_stencil_state.front.passOp = (def->face_culling == CT_FRONT_SIDED) ? VK_STENCIL_OP_INCREMENT_AND_CLAMP
...
void create_standard_pipelines(void) {
	...
	ri.Printf(PRINT_ALL, " Create fog and dlights pipeline \n");
	{
		...
		unsigned int dlight_state_bits[2] = {GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ONE | GLS_DEPTHFUNC_EQUAL,
