...
#define WAVEVALUE(table, base, amplitude, phase, freq)                                                                 \
	((base) + table[(int)((((phase) + tess.shaderTime * (freq)) * FUNCTABLE_SIZE)) & FUNCTABLE_MASK] * (amplitude))
...
void RB_CalcTurbulentTexCoords(const waveForm_t *wf, float *st) {
	...
	for (i = 0; i < tess.numVertexes; i++, st += 2) {
		...
		st[0] =
			s + tr.sinTable[((int)(((tess.xyz[i][0] + tess.xyz[i][2]) * 1.0 / 128 * 0.125 + now) * FUNCTABLE_SIZE)) &
							(FUNCTABLE_MASK)] *
					wf->amplitude;
		st[1] =
			t + tr.sinTable[((int)((tess.xyz[i][1] * 1.0 / 128 * 0.125 + now) * FUNCTABLE_SIZE)) & (FUNCTABLE_MASK)] *
					wf->amplitude;
	}
}
...
void RB_CalcDiffuseColor(unsigned char (*colors)[4]) {
	...
	int numVertexes;
#if idppc_altivec
	vector unsigned char vSel = (vector unsigned char)(0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00,
