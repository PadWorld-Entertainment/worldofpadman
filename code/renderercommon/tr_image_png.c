...
static qboolean EncodeImageNonInterlaced8True(uint32_t IHDR_Width, uint32_t IHDR_Height, byte *InBuffer,
											  uint32_t InBytesPerPixel, uint32_t Padding, uint8_t *ImageData) {
	uint32_t BytesPerScanline, BytesPerPixel, PixelsPerByte;
	uint32_t w, h;
	byte *InPtr;
	uint8_t *OutPtr;
 ...
 */
static void WriteToBuffer(void **buffer, const void *data, size_t length) {
 ...
 */
static void WriteChunkHeader(void **buffer, void **crcPtr, PNG_ChunkCRC *CRC, int type, int length) {
	struct PNG_ChunkHeader CH;
 ...
 * Write CRC to buffer.
 */
static void WriteCRC(void **buffer, void **crcPtr, PNG_ChunkCRC CRC) {
...
#define NUMTEXT 7
	...
	pngSize +=
		PNG_ChunkHeader_Size + compressedDataLength + PNG_ChunkCRC_Size + PNG_ChunkHeader_Size + PNG_ChunkCRC_Size;
