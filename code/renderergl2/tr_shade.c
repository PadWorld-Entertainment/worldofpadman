...
static void DrawNormals(const shaderCommands_t *input) {
	...
	if (vertexCount <= 0)
		return;
	if (!input->shader || input->shader->isSky)
		return; // do not draw over sky/clouds
	if (input->shader->numDeforms > 0)
		return; // skip vertex-deformed surfaces
	...
	if (boundVao) {
		...
		const float lf =
			floatNormals[0] * floatNormals[0] + floatNormals[1] * floatNormals[1] + floatNormals[2] * floatNormals[2];
