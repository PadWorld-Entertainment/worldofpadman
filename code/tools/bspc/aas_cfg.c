...
static const fielddef_t bbox_fields[] = {{"presencetype", BBOX_OFS(presencetype), FT_INT},
										 {"flags", BBOX_OFS(flags), FT_INT},
										 {"mins", BBOX_OFS(mins), FT_FLOAT | FT_ARRAY, 3},
										 {"maxs", BBOX_OFS(maxs), FT_FLOAT | FT_ARRAY, 3},
										 {NULL, 0, 0, 0}};

static const fielddef_t cfg_fields[] = {
...
static const structdef_t bbox_struct = {sizeof(aas_bbox_t), bbox_fields};
static const structdef_t cfg_struct = {sizeof(cfg_t), cfg_fields};
...
static void SetCfgLibVars(void) {
	int i;
	float value;
