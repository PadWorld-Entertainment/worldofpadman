...
static vec_t BoxOriginDistanceFromPlane(vec3_t normal, vec3_t mins, vec3_t maxs, int side) {
	vec3_t v1, v2;
	int i;
...
static vec_t CapsuleOriginDistanceFromPlane(vec3_t normal, vec3_t mins, vec3_t maxs) {
...
static void AAS_ExpandMapBrush(mapbrush_t *brush, vec3_t mins, vec3_t maxs) {
	int sn;
	float dist;
	side_t *s;
	plane_t *plane;
...
static void AAS_SetTexinfo(mapbrush_t *brush) {
	int n;
	side_t *side;
...
static void FreeBrushWindings(mapbrush_t *brush) {
	int n;
	side_t *side;
	//
	for (n = 0; n < brush->numsides; n++) {
...
static void AAS_AddMapBrushSide(mapbrush_t *brush, int planenum) {
	side_t *side;
	//
...
static void AAS_FixMapBrush(mapbrush_t *brush) {
	int i, j, planenum;
	float dist;
	winding_t *w;
...
static qboolean AAS_MakeBrushWindings(mapbrush_t *ob) {
	int i, j;
	winding_t *w;
	side_t *side;
	plane_t *plane, *plane1, *plane2;
...
static mapbrush_t *AAS_CopyMapBrush(mapbrush_t *brush, entity_t *mapent) {
	int n;
	mapbrush_t *newbrush;
	side_t *side, *newside;
...
static int AAS_ValidEntity(entity_t *mapent) {
	int i;
	char target[1024];
...
static int AAS_TransformPlane(int planenum, vec3_t origin, vec3_t angles) {
...
static void AAS_PositionBrush(entity_t *mapent, mapbrush_t *brush) {
	side_t *s;
	float newdist;
	int i, notteam;
	const char *model;
