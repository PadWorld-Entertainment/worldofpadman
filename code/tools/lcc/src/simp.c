...
#define zerofield(OP, TYPE, VAR)                                                                                       \
	...
	return eqtree(OP,                                                                                                  \
				  bittree(BAND, l->kids[0],                                                                            \
						  cnsttree(unsignedtype, (unsigned long)fieldmask(l->u.field) << fieldright(l->u.field))),     \
				  r)
...
static int muli(long x, long y, long min, long max, int needconst) {
	int cond = (x > -1 && x <= 1) || (y > -1 && y <= 1) || (x < 0 && y < 0 && -x <= max / -y) ||
			   (x < 0 && y > 0 && x >= min / y) || (x > 0 && y < 0 && y >= min / x) || (x > 0 && y > 0 && x <= max / y);
	if (!cond && needconst) {
		warning("overflow in constant expression\n");
		cond = 1;
	}
...
Tree simplify(int op, Type ty, Tree l, Tree r) {
	...
	case ADD + P:
		...
		if (isaddrop(l->op) && ((r->op == CNST + I && r->u.v.i <= longtype->u.sym->u.limits.max.i &&
