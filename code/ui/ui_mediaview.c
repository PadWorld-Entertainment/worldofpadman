...
const char *mediatype_strs[] = {":", "picture:", "video:", NULL};
typedef enum {
	MT_UNKNOWN = 0,
	MT_PICTURE,
	MT_VIDEO,
...
static void MediaView_MenuDraw(void) {
	...
	case MT_VIDEO: {
		...
		DrawLine(0, 0, 20, 20, 1,
				 colorBlack); // note: DrawCinematic is no render-cmd, because of that it will probably be drawn before
							  // the previously done 2D-drawings are really drawn (using my Advanced2D, leads to a
							  // scene-randering, which seems to make sure that all 2D things are drawn)
...
void LaunchMediaViewMenu(const char *mediaRef, const char *execOnClose) {
	...
	case MT_VIDEO:
		...
		s_mediaview_menu.CINhandle =
			trap_CIN_PlayCinematic(mediafilepath, 0, 0, 640, 480, /*int bits*/ 0); // CIN_loop, CIN_hold
